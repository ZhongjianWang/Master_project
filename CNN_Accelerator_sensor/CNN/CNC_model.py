...
dataset = read_data('./WISDM_ar_v1.1/magnetdata_cleaned.csv')
...
# X: (?, 1, 90, 3)
X = tf.placeholder(tf.float32, shape=[None, 1, 90, 3], name='X')
# Y: (?, 4)
...
# out_weights: (1000, 6)
out_weights = weight_variable([1000, 5])
# out_biases: (6,)
out_biases = bias_variable([5])
# prediction: (?, 6)
prediction = tf.nn.softmax(tf.matmul(full, out_weights) + out_biases, name='prediction')
# loss: ()
loss = -tf.reduce_sum(Y * tf.math.log(prediction))
optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate = 0.0003).minimize(loss)
